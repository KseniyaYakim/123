...
export default {
  ...
  actions: {
    fetchItems: async ({ commit }) => {
      const response = await api.childrens();
      const items = await response.json();
      commit('setItems', items)
    },
    fetchFiltered: async ({ commit },id) => {
      const response = await api.childrensFiltered(id);
      const items = await response.json();
      commit('setItems', items);
    ...
    addItem: async ({ commit }, { name, age, img_path,country }) => {
      const item = await api.add({ name, age, img_path,country })
      commit('setItem', item)
    },
    updateItem: async ({ commit }, { id, name, age, img_path,country }) => {
      const item = await api.update({ id, name, age,img_path, country });
